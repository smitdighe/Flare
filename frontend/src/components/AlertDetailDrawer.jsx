...
export default function AlertDetailDrawer({ alert, onClose }) {
  ...
  return <motion.aside className="drawer-enter fixed inset-y-0 right-0 z-50 flex w-[420px] max-w-[90vw] flex-col border-l border-line-strong bg-ink-900 shadow-[-20px_0_50px_rgba(0,0,0,0.45)]" initial={{ x: '100%', opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: '100%', opacity: 0 }} transition={{ type: 'spring', stiffness: 260, damping: 28 }} aria-label="Alert evidence drawer"><div className="flex items-start justify-between border-b border-line-strong px-5 py-4"><div><div className="eyebrow mb-2">SIGNAL EVIDENCE // {alert.id}</div><h2 className="max-w-[300px] text-xl font-semibold leading-tight text-paper">{alert.attack_type?.replaceAll('_', ' ')}</h2><div className="mt-2 flex items-center gap-2 font-mono-ui text-[10px] text-ash"><StatusDot tone={alert.severity} pulse={alert.severity === 'critical'} />{alert.severity?.toUpperCase()} // {alert.protocol}:{alert.dest_port}</div></div><button ref={closeRef} type="button" className="ghost-button flex h-8 w-8 items-center justify-center" onClick={onClose} aria-label="Close evidence drawer"><Icon name="close" size={17} /></button></div><div className="flex-1 overflow-y-auto">
    ...
    <section className="drawer-section px-5 py-4" style={{ '--section-index': 5 }}><SectionHeader label="RECOMMENDED ACTION" open={openSections.action} onToggle={() => toggle('action')} /><AnimatePresence initial={false}>{openSections.action && <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden"><p className="mt-3 border-l-2 border-amber bg-amber/5 px-3 py-2 text-xs leading-5 text-paper">{alert.remediation || 'Maintain watch state and await additional evidence.'}</p><div className="mt-3 flex items-center gap-2"><select value={selectedPb} onChange={(e) => setSelectedPb(e.target.value)} className="flex-1 border border-line-strong bg-ink-950 px-2 py-1.5 font-mono-ui text-[10px] text-paper">{playbooks.length === 0 && <option value="">No playbooks</option>}{playbooks.map((pb) => <option key={pb.id} value={pb.id}>{pb.name}</option>)}</select><button type="button" className="terminal-button inline-flex items-center gap-2 px-3 py-1.5 font-mono-ui text-[10px] uppercase tracking-[0.08em]" onClick={handleQueueAction}>Queue action <Icon name="arrow_forward" size={14} /></button></div>{queueMsg && <div className="mt-2 font-mono-ui text-[10px] text-amber">{queueMsg}</div>}</motion.div>}</AnimatePresence></section>
  </div></motion.aside>;
}
